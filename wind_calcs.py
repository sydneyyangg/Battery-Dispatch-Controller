...
def get_weather_data(retries=3):
    r"""
    ...
    variables = ['temp_air', '10m_u_component_of_wind', '10m_v_component_of_wind', '100m_u_component_of_wind', '100m_v_component_of_wind']
    api_key = ''
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            weather['wind_speed_10'] = np.hypot(
                weather['u10'],
                weather['v10'],
            )
            weather['wind_speed_100'] = np.hypot(
                weather['u100'],
                weather['v100'],
            )
            ...
            weather = weather[['pressure', 'temperature', 'wind_speed_10', 'wind_speed_100']].copy()
            roughness_length = 0.3
            weather['roughness_length'] = roughness_length
            weather = weather[['pressure', 'temperature', 'wind_speed_10', 'wind_speed_100', 'roughness_length']].copy()

            weather.columns = pd.MultiIndex.from_tuples(
                [('pressure', 0), ('temperature', 2), ('wind_speed', 10), ('wind_speed', 100), ('roughness_length', 0)],
    ...
    weather = weather.rename(columns={'temp_air': 'temperature'})
    if 'wind_speed_10' not in weather.columns:
        weather['wind_speed_10'] = np.hypot(weather.get('u10', 0), weather.get('v10', 0))
    if 'wind_speed_100' not in weather.columns:
        weather['wind_speed_100'] = np.hypot(weather.get('u100', 0), weather.get('v100', 0))
    weather = weather[['pressure', 'temperature', 'wind_speed_10', 'wind_speed_100']].copy()
    roughness_length = 0.3
    weather['roughness_length'] = roughness_length
    weather = weather[['pressure', 'temperature', 'wind_speed_10', 'wind_speed_100', 'roughness_length']].copy()
...
def calculate_power_output(weather, my_turbine, e126, my_turbine2):
    ...
    my_turbine2 : :class:`~.wind_turbine.WindTurbine`
        WindTurbine object with power curve from example file.
    ...
    annual_energy_e126 = mc_e126.power_output.sum() / 1000
    print(f"Enercon E126 annual energy: {annual_energy_e126:.2f} kWh")
    # write power output time series to WindTurbine object
    e126.power_output = mc_e126.power_output.clip(upper=e126.nominal_power)
    # ************************************************************************
    # **** ModelChain with default parameter *********************************
    mc_my_turbine = ModelChain(my_turbine).run_model(weather)
    annual_energy_my_turbine = mc_my_turbine.power_output.sum() / 1000
    print(f"myTurbine annual energy: {annual_energy_my_turbine:.2f} kWh")
    # write power output time series to WindTurbine object
    my_turbine.power_output = mc_my_turbine.power_output.clip(upper=my_turbine.nominal_power)
    ...
    annual_energy_my_turbine2 = mc_example_turbine.power_output.sum() / 1000
    print(f"myTurbine2 annual energy: {annual_energy_my_turbine2:.2f} kWh")
    my_turbine2.power_output = mc_example_turbine.power_output.clip(upper=my_turbine2.nominal_power)
